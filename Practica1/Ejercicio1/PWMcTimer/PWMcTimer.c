// Este ejemplo te permite ver como se puede generar una señal PWM usando los timers del sistema.
// No obstante, SE RECOMIENDA usar los modulos PWM de la TIVA para generar seÃ±ales PWM en vez de los Timers (ver EjemploPWM)
...
int main(void) {
    ...
    ui32Period = PWMCYCLE ; // Periodo de la seÃ±al PWM; 1000/40MHz= 25uS
    ui32DutyCycle = DUTYCYCLE99;// Ciclo de trabajo inicial de la seÃ±al PWM
    ...
    GPIOPinTypePWM(GPIO_PORTF_BASE, GPIO_PIN_1); // Observa que aqui se usa otro "formato" para indicar el pin afectado. Consulta la documentaciï¿½n de la funciï¿½n
    ...
    TimerControlLevel(TIMER0_BASE, TIMER_B, true); // Invierte la señal PWM para que a mayor ui32DutyCycle, mas brillo
